...
export const OnlineUsersList: React.FC<OnlineUsersListProps> = ({
  ...
  if (loading) {
    return (
      <div
        className={`bg-dark-secondary rounded-lg shadow-sm border border-slate-700 ${className}`}
      >
        <div className="p-4 border-b border-slate-700">
          <h3 className="font-semibold text-white">{t("chat:onlineUsers")}</h3>
        </div>
  ...
  if (error) {
    return (
      <div
        className={`bg-dark-secondary rounded-lg shadow-sm border p-6 text-center border-slate-700 ${className}`}
      >
  ...
  return (
    ...
    >
      <div className="p-4 border-b border-slate-700">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              {loading ? (
                <span className="inline-block animate-spin w-3 h-3 border border-gray-300 border-t-transparent rounded-full" />
              ) : (
                "⟳"
              )}
            </button>
      ...
      {/* refresh control moved to header */}
    </div>
  );
};
