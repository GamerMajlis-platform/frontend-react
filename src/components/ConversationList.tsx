...
export default function ConversationList({ conversations, selectedId, onSelect }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter((c) =>
      [c.name, c.lastMessage].join(" ").toLowerCase().includes(query.toLowerCase())
    );
  }, [conversations, query]);
      ...
      <div className="sticky top-4">
        ...
        <div className="flex flex-col gap-2">
          {filtered.map((c) => (
            ...
            >
              <div className="w-12 h-12 rounded-full bg-slate-700 flex items-center justify-center text-white font-bold">{c.name.split(" ")[0][0] ?? "U"}</div>
              ...
              {c.unread ? (
                <div className="ml-2 text-sm bg-cyan-300 text-slate-900 rounded-full w-6 h-6 flex items-center justify-center">{c.unread}</div>
              ) : null}
            </button>
          ))}
        </div>
