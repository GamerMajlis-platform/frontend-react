...
import type { DiscordLoginButtonProps } from "../../types";

/**
 * Discord Login Button Component
 * Provides Discord OAuth authentication for login/signup
 */
export const DiscordLoginButton: React.FC<DiscordLoginButtonProps> = ({
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className={`
        flex items-center justify-center gap-3 rounded-lg font-medium
    ...
    >
      ...
      {isLoading ? (
        <div className="flex items-center gap-2">
          ...
          {t("discord.auth.connecting")}
        </div>
      ) : (
        t("discord.auth.loginWithDiscord")
      )}
    </button>
  );
};
